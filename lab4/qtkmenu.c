...
{
	GtkWidget *window;
	GtkWidget *menubar, *fileitem;GtkWidget *filemenu, *newitem, *openitem;GtkWidget *submenu,*subitem;GtkWidget *vbox, *message_label;
	gtk_init(&argc, &argv);
	...
	message_label= gtk_label_new ("20163329천정석");
	gtk_box_pack_start (GTK_BOX (vbox), menubar, FALSE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX (vbox), message_label, TRUE, FALSE, 0);
	fileitem= gtk_menu_item_new_with_mnemonic ("파일");
	gtk_container_add (GTK_CONTAINER (menubar), fileitem);filemenu= gtk_menu_new ();
	gtk_menu_item_set_submenu (GTK_MENU_ITEM (fileitem), filemenu);newitem= gtk_menu_item_new_with_mnemonic ("생성");
